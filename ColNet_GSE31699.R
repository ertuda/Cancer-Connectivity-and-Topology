...
setwd('C:/Users/Dell/Documents/Projects/Colorectal Topology/GSE31699/')
agd <- read.table('GPL6947-GeneID.txt',header=TRUE)

e31699ad <- read.table('GSE31699_adenoma.txt',header=TRUE,as.is=TRUE)
e31699ad$ID <- sapply(e31699ad$ID,function(i) agd[agd$ImnID==i,]$GeneID)
...
axn <- apply(e31699ad[,e31699adns+1],1,sd)
axt <- apply(e31699ad[,e31699adts+1],1,sd)
ax <- apply(cbind(axn,axt),1,max)
names(ax) = e31699ad$ID
sax <- split(ax,as.factor(names(ax)))
ssax <- sapply(sax,max)
sind <- sapply(seq_along(ax),function(i) ax[i]==ssax[names(ax)[i]])
e31699ad <- e31699ad[sind,]

rownames(e31699ad) = e31699ad$ID
e31699ad <- e31699ad[,-1]

# next, we reduce genes by average expression level
# to avoid lowly expressed genes from correlation based analysis
lowexpthr = 5
me31699ad <- rowMeans(e31699ad)
e31699adh <- e31699ad[me31699ad>=lowexpthr,]

# next, we reduce genes by variance
# to avoid lowly varied genes from correlation based analysis
lowvarthr = 0.5
de31699adhn <- apply(e31699adh[,e31699adns],1,sd)
de31699adht <- apply(e31699adh[,e31699adts],1,sd)
de31699adh <- apply(cbind(de31699adhn,de31699adht),1,max)
e31699adhv <- e31699adh[de31699adh>=lowvarthr,]
print(paste(nrow(e31699adhv),' genes are selected for GSe31699 adenoma'))
...
noradeexp <- e31699adhv[,e31699adns];tumadeexp <- e31699adhv[,e31699adts]
...
for (jj in 1:norands){
  ...
  sc = c(1:e31699adts[length(e31699adts)])
  randset = sample(sc,e31699adns[length(e31699adns)])
  randset2 = sc[-randset]
  randnoradeexp <- e31699adhv[,randset];randtumadeexp <- e31699adhv[,randset2]
...
write.table(lessapd,file='lessapd_Leiomyoma.txt',row.names=TRUE,col.names=FALSE,sep=',',quote=FALSE)
write.table(moreapd,file='moreapd_Leiomyoma.txt',row.names=TRUE,col.names=FALSE,sep=',',quote=FALSE)
write.table(lessand,file='lessand_Leiomyoma.txt',row.names=TRUE,col.names=FALSE,sep=',',quote=FALSE)
write.table(moreand,file='moreand_Leiomyoma.txt',row.names=TRUE,col.names=FALSE,sep=',',quote=FALSE)
...
write.table(lessapd5,file='lessapd_top5percent_Leiomyoma.txt',row.names=TRUE,col.names=FALSE,sep=',',quote=FALSE)
write.table(lessapd10,file='lessapd_top10percent_Leiomyoma.txt',row.names=TRUE,col.names=FALSE,sep=',',quote=FALSE)
write.table(lessapd20,file='lessapd_top20percent_Leiomyoma.txt',row.names=TRUE,col.names=FALSE,sep=',',quote=FALSE)

perc5 = quantile(moreapd,probs=0.95)
perc10 = quantile(moreapd,probs=0.9)
perc20 = quantile(moreapd,probs=0.8)
write.table(moreapd[moreapd>=perc5],file='moreapd_top5percent_Leiomyoma.txt',row.names=TRUE,col.names=FALSE,sep=',',quote=FALSE)
write.table(moreapd[moreapd>=perc10],file='moreapd_top10percent_Leiomyoma.txt',row.names=TRUE,col.names=FALSE,sep=',',quote=FALSE)
write.table(moreapd[moreapd>=perc20],file='moreapd_top20percent_Leiomyoma.txt',row.names=TRUE,col.names=FALSE,sep=',',quote=FALSE)

perc5 = quantile(lessand,probs=0.95)
perc10 = quantile(lessand,probs=0.9)
perc20 = quantile(lessand,probs=0.8)
write.table(lessand[lessand>=perc5],file='lessand_top5percent_Leiomyoma.txt',row.names=TRUE,col.names=FALSE,sep=',',quote=FALSE)
write.table(lessand[lessand>=perc10],file='lessand_top10percent_Leiomyoma.txt',row.names=TRUE,col.names=FALSE,sep=',',quote=FALSE)
write.table(lessand[lessand>=perc20],file='lessand_top20percent_Leiomyoma.txt',row.names=TRUE,col.names=FALSE,sep=',',quote=FALSE)

perc5 = quantile(moreand,probs=0.95)
perc10 = quantile(moreand,probs=0.9)
perc20 = quantile(moreand,probs=0.8)
write.table(moreand[moreand>=perc5],file='moreand_top5percent_Leiomyoma.txt',row.names=TRUE,col.names=FALSE,sep=',',quote=FALSE)
write.table(moreand[moreand>=perc10],file='moreand_top10percent_Leiomyoma.txt',row.names=TRUE,col.names=FALSE,sep=',',quote=FALSE)
write.table(moreand[moreand>=perc20],file='moreand_top20percent_Leiomyoma.txt',row.names=TRUE,col.names=FALSE,sep=',',quote=FALSE)
...
write.table(lessapd[lessapd>=perc5],file='lessapd_top5percent_Leiomyoma.txt',row.names=TRUE,col.names=FALSE,sep=',',quote=FALSE)
write.table(lessapd[lessapd>=perc10],file='lessapd_top10percent_Leiomyoma.txt',row.names=TRUE,col.names=FALSE,sep=',',quote=FALSE)
write.table(lessapd[lessapd>=perc20],file='lessapd_top20percent_Leiomyoma.txt',row.names=TRUE,col.names=FALSE,sep=',',quote=FALSE)



# plot degs and deg differences
png(file='adeno_carci_degs_Leiomyoma.png',width=240,height=600)
par(mfrow = c(2,1))
